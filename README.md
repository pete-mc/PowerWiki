...
- Split view scrolls in step: the pane you scroll moves the other to the matching place, interpolating across tall blocks so a Mermaid diagram or a long table does not throw the alignment out.
- Resize images by dragging their handles in the rich text editor (GitHub #35). Corners keep the proportions, edges squash deliberately, and the size is written back as Azure DevOps' own `=WIDTHxHEIGHT` suffix, so the page still renders correctly in the built-in wiki.
